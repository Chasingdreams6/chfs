...
#define BITMAP_BIOS(block) ((block - (block / BPB) * BPB) >> 3)
#define BIT_SHIFT(block) ((block - (block / BPB) * BPB) % 8)
#define SHIFTED(block) ((1 << (BIT_SHIFT(block))) & 0xff)
#define SHIFTED_0(block) ((~ (1 << BIT_SHIFT(block))) & 0xff)
#define CHECK_USED(block, ch) ((ch >> (BIT_SHIFT(block)) & 1))
#define DEBUG_MODE 0
...
block_manager::alloc_block()
{
  ...
  while (startBlock < BLOCK_NUM) {
    if (using_blocks.find(startBlock) != using_blocks.end()) {
        startBlock++;
        continue;
    }
